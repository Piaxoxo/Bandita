...
// Cheap, GPU-friendly 3D tilt: sets transform directly, capped to one update
// per frame (no per-mousemove tween). Reset eases back with a CSS transition.
...
const HERO_CLUSTER = ["/work/film-01.jpg", "/portfolio/plein/01.jpg", "/work/food-01.jpg"];
...
export default function ServicesExperience({ lang, dict }: { lang: Locale; dict: Dictionary }) {
  ...
  return (
    <div className="relative bg-creme text-ink">
        ...
        <div className="relative mx-auto grid w-full max-w-[1500px] items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
          ...
          <div>
            ...
            <Reveal className="mt-9 flex flex-wrap items-center gap-4">
              <MagneticButton onClick={() => scrollToId("preise")} strength={0.5}
                className="rounded-full bg-ink px-8 py-4 font-sans text-sm uppercase tracking-[0.14em] text-creme transition-colors hover:bg-pink">
          ...
          <div className="relative hidden lg:block" style={{ perspective: "1400px" }}>
            <div ref={hero.ref} onMouseMove={hero.onMove} onMouseLeave={hero.onLeave}
              ...
              <img src={HERO_CLUSTER[0]} alt="Bandita — Filmproduktion" loading="eager" decoding="async"
                className="absolute left-[6%] top-[4%] h-[62%] w-[62%] rounded-2xl object-cover shadow-[0_40px_90px_-30px_rgba(20,12,18,0.6)]"
                style={{ transform: "translateZ(60px)" }} />
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={HERO_CLUSTER[1]} alt="Bandita — Fashion" loading="eager" decoding="async"
                className="absolute right-[2%] top-[26%] h-[52%] w-[46%] rounded-2xl border-4 border-creme object-cover shadow-[0_40px_90px_-30px_rgba(20,12,18,0.6)]"
                style={{ transform: "translateZ(120px)" }} />
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={HERO_CLUSTER[2]} alt="Bandita — Food" loading="lazy" decoding="async"
                className="absolute bottom-[2%] left-[18%] h-[40%] w-[42%] rounded-2xl border-4 border-creme object-cover shadow-[0_40px_90px_-30px_rgba(20,12,18,0.6)]"
                style={{ transform: "translateZ(30px)" }} />
              <span aria-hidden className="absolute -right-2 top-[16%] h-24 w-24 rounded-2xl bg-pink" style={{ transform: "translateZ(-10px)" }} />
            </div>
          </div>
        </div>
      ...
      {/* ② STICKY JUMP-NAV — jump straight to what you need */}
      <div className="sticky top-[58px] z-40 border-y border-ink/10 bg-creme/85 backdrop-blur-md">
        <div className="mx-auto flex max-w-[1500px] items-center gap-1 overflow-x-auto px-4 py-2.5 md:justify-center md:px-10">
          {NAV.map((n) => (
      ...
      {/* ③ LEISTUNGEN — filterable catalogue (top priority) */}
      ...
      {/* ④ PREISE — filterable, real numbers */}
              ...
              return (
                <div key={t.id}
                  ...
                  <ul className={`mt-5 flex-1 space-y-2 font-sans text-[14px] ${feat ? "text-creme/80" : "text-ink/70"}`}>
                    {t.features.map((f, fi) => (
                      <li key={fi} className="flex items-start gap-2.5"><span className="mt-1.5 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-pink" />{f}</li>
                    ))}
                  </ul>
      ...
      {/* ⑦ DISZIPLINEN — the visual "wow" grid (3D tilt cards) */}
      <section id="disziplinen" className="relative px-5 py-20 md:px-10 md:py-28">
        <div className="mx-auto max-w-[1500px]">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4" style={{ perspective: "1200px" }}>
            {CHAPTERS.map((c) => (
              <DisciplineCard key={c.id} c={c} lang={lang} reduced={r} />
            ))}
          </div>
        </div>
      </section>
      ...
      {/* ⑨ AI STUDIO — flagship, near the bottom */}
...
// A discipline card: full-bleed on-theme photo, 3D tilt on hover, links into
// the portfolio. Hook lives at the top level of its own component (not in a map).
      ...
      <div className="absolute inset-x-5 bottom-5">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
          {c.services.slice(0, 4).map((s) => (
            <span key={s} className="rounded-full border border-creme/25 px-2 py-0.5 font-sans text-[10px] text-creme/85">{s}</span>
          ))}
        </div>
...
// Tilt for the discipline cards (returns a ref usable on <Link>).
